...
// FIX SERVO ISSUE
// Try to add new method on Servo class
// Idea: reset X timer in each loop
void move(int* btCmd, const int cmd, Servo & servoObj, int* servoP, const bool counterclockwise, int* velocity) {
   while (btCmd == cmd) {
      if (Bluetooth.available()) {
        btCmd = Bluetooth.read();
      }
      servoObj.write(servoP);
...
void loop() {
  ...
  if (Bluetooth.available()) {
    ...
    move(btCmd, 16, servo01, servo1PPos, true, velocity);
    ...
    if (btCmd == 14) {
      runSteps(); //Start the void "runSteps" which runs the steps in sequence
...
// Special (and separate) function to make the steps go automatically
void runSteps() {
